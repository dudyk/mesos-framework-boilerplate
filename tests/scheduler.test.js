...
describe("Scheduler constructor", function() {
    ...
    describe("Create scheduler with ZK", function () {
        ...
        beforeEach(function () {
            ...
            sandbox.stub(taskHelper, "loadTaskDefs", function (tasks) {
                var self = this;
...
describe("Request functions", function() {
    ...
    it("sync Success", function(done) {
        ...
        var sent = 0;
        scheduler.on("sent_reconcile", function(tasks) {
            if (tasks) {
                sent += tasks;
            } else {
                sent += 1;
        ...
        setTimeout(function () {
            ...
            expect(scheduler.killTasks).to.have.lengthOf(0);
            expect(scheduler.reconcileTasks).to.have.lengthOf(0);
            done();
        }, 400);
        clock.tick(100);
        clock.tick(400);
...
describe("Subscribe flow", function() {
    ...
    it("OK http status - with stream id, valid JSON response, valid type - HOST and PORT set (subscribed)", function(done) {
        ...
        var req = new MockReq({ method: "POST" });
        this.request.callsArgWith(1, res).returns(req);
        envs.PORT0 = process.env.PORT0 = "1234";
        envs.HOST= process.env.HOST = "localhost";
    ...
    it("OK http status - with stream id, valid JSON response, valid type and timeout with DNS (subscribed)", function(done) {
            ...
            //res.end();
            var req = new MockReq({ method: "POST" });
            this.request.onFirstCall().callsFake(function (sentOptions, cb) {
                console.log("request stub called");
                cb(res);
                return req;
            });//.callsArgWith(1, res).returns(req);
            this.request.onSecondCall().callsFake(function (sentOptions, cb) {
                console.log("request stub called a second time");
                cb(res2);
                return req;
            });//.callsArgWith(1, res2).returns(req);
            var scheduler = new Scheduler({tasks: {
            ...
            setTimeout(function () {
                clock.tick(scheduler.subscribeBackoffTime);          
    ...
    it("OK http status - with stream id, valid JSON response, valid type and timeout with redirect and timeout (fail)", function(done) {
        function SocketStub() {
            ...
            this.request.onSecondCall().callsFake(function (sentOptions, cb) {
                console.log("request stub called second time");
                cb(res2);
                return req2;
            });//.callsArgWith(1, res2).returns(req2);
            this.request.onThirdCall().callsFake(function (sentOptions, cb) {
                console.log("request stub called 3rd time");
                cb(res3);
                return req3;
            });//.callsArgWith(1, res3).returns(req3);
            var scheduler = new Scheduler({tasks: {
                task1:{isSubmitted:true}},useZk: false, logging: {level: "debug"}});
    ...
    it.skip("OK http status - with stream id, valid JSON response, valid type and timeout with DNS (subscribed)", function(done) {
            function SocketStub() {
                // Inherit from EventEmitter
                EventEmitter.call(this);
                return this;
...
describe("Subscribe with ZK", function () {
    ...
    it("Success path (saved in ZK, with options)", function (done) {
        var data = "2\n{\"type\":\"SUBSCRIBED\",\"subscribed\":{\"framework_id\":{\"value\":\"122353532\"}}}";
        var res = new MockRes();
        var errorSet = false;
        var called = false;
        ...
        var scheduler = new Scheduler({tasks: {
            task1:{isSubmitted:true}
        }, useZk: true, configVersion: "1.00",
        logging: {level: "debug"}, zkClient: zkClient, taskHelper: taskHelper});
        scheduler.on("error", function(error) {
            console.log(JSON.stringify(error));
            errorSet = true;
        });
    ...
    it("Success path (saved in ZK, with options and handler)", function (done) {
        var data = "2\n{\"type\":\"SUBSCRIBED\",\"subscribed\":{\"framework_id\":{\"value\":\"122353532\"}}}";
        var res = new MockRes();
        var errorSet = false;
        var called = false;
        ...
        logging: {level: "debug"}, zkClient: zkClient, taskHelper: taskHelper,
        optionsHandler: function (options, cb) {
